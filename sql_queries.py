...
INSERT INTO Institutions_IPEDS
    (UNITID, INSTNM, ADDR, CITY, STABBR, ZIP,
     LATITUDE, LONGITUDE,
     C21BASIC, C21IPUG, C21UGPRF, C21ENPRF, C21SZSET,
     COUNTYCD, CSA, CBSA,
     LAST_REPORTED, LAST_UPDATED)
VALUES
    (%s, %s, %s, %s, %s, %s,
...
SET
    ...
    LAST_UPDATED  = CASE
        WHEN  Institutions_IPEDS.INSTNM     IS DISTINCT FROM EXCLUDED.INSTNM
           ...
           OR Institutions_IPEDS.C21BASIC   IS DISTINCT FROM EXCLUDED.C21BASIC
           OR Institutions_IPEDS.C21IPUG    IS DISTINCT FROM EXCLUDED.C21IPUG
           OR Institutions_IPEDS.C21UGPRF   IS DISTINCT FROM EXCLUDED.C21UGPRF
           OR Institutions_IPEDS.C21ENPRF   IS DISTINCT FROM EXCLUDED.C21ENPRF
           OR Institutions_IPEDS.C21SZSET   IS DISTINCT FROM EXCLUDED.C21SZSET
...
WHERE
    ...
    OR Institutions_IPEDS.CSA        IS DISTINCT FROM EXCLUDED.CSA
    OR Institutions_IPEDS.CBSA       IS DISTINCT FROM EXCLUDED.CBSA
    OR Institutions_IPEDS.LAST_REPORTED IS DISTINCT FROM EXCLUDED.LAST_REPORTED;
"""
